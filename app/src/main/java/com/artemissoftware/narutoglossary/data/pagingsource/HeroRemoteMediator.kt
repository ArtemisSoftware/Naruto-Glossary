...
class HeroRemoteMediator @Inject constructor(
    ...
    private val heroRemoteKeysDao = narutoGlossaryDataBase.heroRemoteKeysDao()


    override suspend fun initialize(): InitializeAction {
        val currentTime = System.currentTimeMillis()
        val lastUpdated = heroRemoteKeysDao.getRemoteKeys(heroId = 1)?.lastUpdated ?: 0L
        val cacheTimeout = 1//1440
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Hero>): MediatorResult {
        return try {
            ...
            val response = narutoGlossaryApi.getAllHeroes(page = page)
            if (response.heroes.isNotEmpty()) {
                narutoGlossaryDataBase.withTransaction {
